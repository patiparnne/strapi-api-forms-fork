...
const JoditInput: React.FC<JoditInputProps> = ({
  ...
  const mediaLibButton = {
    ...
    exec: function (jodit: IJodit) {
      ...
      onChange(newContent.split(cursorPlaceholderContent).join('').trim());
      toggleMediaLib();
    }
  }
  ...
  const insertPlaceholder = (field: string) => {
    ...
    if (jodit) {
      ...
      jodit.selection.insertHTML(placeholder);
      
      // Trigger onChange to update the parent component
      const newContent = jodit.value;
      onChange(newContent);
      
      console.log('📎 Jodit: Inserted placeholder', placeholder);
    }
  };

  const height = 400;
  const buttons: any[] = DEFAULT_BUTTONS.split(',').map(btn => btn.trim());
  ...
  return (
    <DesignSystemProvider>
      ...
      <Field.Root
        name='APIFormsJoditInput'
        id='api-forms-jodit-input'
